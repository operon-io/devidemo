...
public class Json {
	...
	public static com.altova.json.Value TagValue(String tag, com.altova.mapforce.IMFNode item)
    {
		com.altova.json.Value x = (com.altova.json.Value)item;
	...
	static void Write(com.altova.json.Value value, com.altova.io.Output target, boolean prettyPrint, boolean json5) throws Exception {
		switch (target.getType()) {
			case com.altova.io.Output.IO_STREAM:
				//try (FileIO io = new FileIO(target.getStream(), null, false, false)) { // CloseObjectsAfterRun property controls whether to close stream, do not auto close it with FileIO!
		...
		if (target.getType() != com.altova.io.Output.IO_WRITER && target.getType() != com.altova.io.Output.IO_STREAM)
			...
			if (jsonLines) {
				if (target.getType() == com.altova.io.Output.IO_STREAM)
					//try (FileIO io = new FileIO(target.getStream(), null, false, false)) { // CloseObjectsAfterRun property controls whether to close stream, do not auto close it with FileIO!
						WriteJsonLines(en, (new FileIO(target.getStream(), null, false, false)).openWriteStream(), json5);
					//}
				else
